...
<envoy_api_field_Cluster.health_checks>` for a cluster, all upstream cluster members are considered
healthy.
...
from the cluster. No other :ref:`load balancing type <envoy_api_field_Cluster.lb_policy>` can
