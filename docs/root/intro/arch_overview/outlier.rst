...
#. Envoy checks to make sure the number of ejected hosts is below the allowed threshold (specified
   ...
   <envoy_api_msg_Cluster.OutlierDetection>` setting).
...
#. The host is ejected for some number of milliseconds. Ejection means that the host is marked
   ...
   <envoy_api_msg_Cluster.OutlierDetection>` value
...
* Cluster manager :ref:`global configuration <envoy_api_field_ClusterManager.outlier_detection>`
* Per cluster :ref:`configuration <envoy_api_field_Cluster.outlier_detection>`
